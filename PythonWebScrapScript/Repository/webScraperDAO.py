...
class webScraperDAO:
      ...
      def insertPost(self, table, post):
          ...
          val = (str(post.url), post.topic, post.socialMediaSource,
                 ...
                 post.PosterName, str(post.PostText)
      ...
      def insertComment(self, table, comment):
          ...
          val = (comment.postId, comment.commenterID, comment.commentDate,
                 comment.commenterName, str(comment.commentText), comment.replyToName,
                  comment.replyToID, comment.commentFloor
      ...
      def getCommentText(self, table):
          self.cursor.execute('''select distinct CONVERT(commentText USING utf8) from lihkgcomment comment
                              inner join lihkgpost post
                              on comment.PostID = post.PostID
                              where SocialMediaSource is Null''')
